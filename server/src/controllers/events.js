...
export const add_event = async (req,res) => {
    try {
        const query = "INSERT INTO event (title, location, startEvent, endEvent, createdAt, isFinished, category_id, user_id) VALUES (?, ?, ?, ?, NOW(), 0, ?, ?)";
...
export const all = async (req, res) => {
    try {
        const query = "SELECT event.id, title, location, startEvent, endEvent, createdAt, isFinished, category_id, user_id FROM event";
...
export const one = async (req, res) => {
    try {
        const queryEvent = "SELECT event.id, title, location, startEvent, endEvent, createdAt, isFinished, category_id FROM event WHERE id = ?";
...
export const update = async (req,res) => {
    try {
        const query = "UPDATE event SET title = ?, location = ?, startEvent = ?, endEvent = ?, isFinished = 0, category_id = ? WHERE id = ?";
        const [result] = await Query.write(query, req.body);
